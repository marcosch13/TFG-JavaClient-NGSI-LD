...
public class EntityScope extends AbstractOpenApiSchema {
    private static final Logger log = Logger.getLogger(EntityScope.class.getName());

    public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
        @SuppressWarnings("unchecked")
        @Override
        public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
            if (!EntityScope.class.isAssignableFrom(type.getRawType())) {
                return null; // this class only serializes 'EntityScope' and its subtypes
            }
            ...
            return (TypeAdapter<T>) new TypeAdapter<EntityScope>() {
                @Override
                public void write(JsonWriter out, EntityScope value) throws IOException {
                ...
                @Override
                public EntityScope read(JsonReader in) throws IOException {
                    ...
                    if (match == 1) {
                        EntityScope ret = new EntityScope();
                        ret.setActualInstance(actualAdapter.fromJsonTree(jsonElement));
                        return ret;
                    }

                    throw new IOException(String.format("Failed deserialization for EntityScope: %d classes match result, expected 1. Detailed failure message for oneOf schemas: %s. JSON: %s", match, errorMessages, jsonElement.toString()));
                }
            }.nullSafe();
        }
    }
     ...
     * @throws IOException if the JSON Element is invalid with respect to EntityScope
     */
    public static void validateJsonElement(JsonElement jsonElement) throws IOException {
        ...
        if (validCount != 1) {
            throw new IOException(String.format("The JSON string is invalid for EntityScope with oneOf schemas: List<String>, String. %d class(es) match the result, expected 1. Detailed failure message for oneOf schemas: %s. JSON: %s", validCount, errorMessages, jsonElement.toString()));
        }
    }

    /**
     * Create an instance of EntityScope given an JSON string
     *
     * @param jsonString JSON string
     * @return An instance of EntityScope
     * @throws IOException if the JSON string is invalid with respect to EntityScope
     */
    public static EntityScope fromJson(String jsonString) throws IOException {
        return JSON.getGson().fromJson(jsonString, EntityScope.class);
    }

    /**
     * Convert an instance of EntityScope to an JSON string
     *
     * @return JSON string
     */
