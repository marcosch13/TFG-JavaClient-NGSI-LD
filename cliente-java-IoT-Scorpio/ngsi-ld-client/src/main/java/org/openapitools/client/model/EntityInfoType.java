...
public class EntityInfoType extends AbstractOpenApiSchema {
    private static final Logger log = Logger.getLogger(EntityInfoType.class.getName());

    public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
        @SuppressWarnings("unchecked")
        @Override
        public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
            if (!EntityInfoType.class.isAssignableFrom(type.getRawType())) {
                return null; // this class only serializes 'EntityInfoType' and its subtypes
            }
            ...
            return (TypeAdapter<T>) new TypeAdapter<EntityInfoType>() {
                @Override
                public void write(JsonWriter out, EntityInfoType value) throws IOException {
                ...
                @Override
                public EntityInfoType read(JsonReader in) throws IOException {
                    ...
                    if (match == 1) {
                        EntityInfoType ret = new EntityInfoType();
                        ret.setActualInstance(actualAdapter.fromJsonTree(jsonElement));
                        return ret;
                    }

                    throw new IOException(String.format("Failed deserialization for EntityInfoType: %d classes match result, expected 1. Detailed failure message for oneOf schemas: %s. JSON: %s", match, errorMessages, jsonElement.toString()));
                }
            }.nullSafe();
        }
    }
    ...
    public EntityInfoType(Object o) {
        super("oneOf", Boolean.FALSE);
        setActualInstance(o);
    }
     ...
     * @throws IOException if the JSON Element is invalid with respect to EntityInfoType
     */
    public static void validateJsonElement(JsonElement jsonElement) throws IOException {
        ...
        if (validCount != 1) {
            throw new IOException(String.format("The JSON string is invalid for EntityInfoType with oneOf schemas: List<String>, String. %d class(es) match the result, expected 1. Detailed failure message for oneOf schemas: %s. JSON: %s", validCount, errorMessages, jsonElement.toString()));
        }
    }

    /**
     * Create an instance of EntityInfoType given an JSON string
     *
     * @param jsonString JSON string
     * @return An instance of EntityInfoType
     * @throws IOException if the JSON string is invalid with respect to EntityInfoType
     */
    public static EntityInfoType fromJson(String jsonString) throws IOException {
        return JSON.getGson().fromJson(jsonString, EntityInfoType.class);
    }

    /**
     * Convert an instance of EntityInfoType to an JSON string
     *
     * @return JSON string
     */
